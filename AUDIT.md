...
**新しい試験に判別力があることも確かめています。** v1.8.16 の実装（`src/content.js` をタグの内容へ戻したもの）へぶつけると、**25件中17件が失敗**します（うち1件は撤回した挙動を固定していた第17回の試験を書き直したもの）。通る9件の内訳は、**落としすぎを見張る対照が7件**と、**v1.8.16 も同じ答えを返す補助の確認が2件**（画面外の Tab 停止点／縦書きの下側）です。
