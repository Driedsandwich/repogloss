...
**⚠️ この版では、しきい値（初期走査 50ms 未満）を実測で示せていません。** 測定した機械が別の作業で混んでおり、**比較対象の v1.8.4 も同じように 50ms を超えた**ためです。数字をそのまま載せます。

**同じ機械で交互に測った比較**（`k88hudson/git-flight-rules`・幅1600px・A と B を1回ずつ交互に5往復・ページの中で初期走査だけを計測）:
