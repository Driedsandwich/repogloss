...
| ストア公開中の版 | **v1.7.1**（v1.8.0〜v1.8.16 はいずれも未提出） |
...
第17回監査の指摘 **P1 4件・P2 4件**の**全8件**への対応。詳細と証拠は [`docs/audit/v1.8.16-changes.md`](docs/audit/v1.8.16-changes.md)。

| ID | 内容 | 対応 |
|---|---|---|
| RG-17-01（P1） | スクロールの可動域を両側へ対称に見積もる | 軸ごとの実際の可動域（原点は片側）から到達範囲を出す |
| RG-17-02（P1） | 角丸との交差を、角の箱へ**完全に収まる**ときしか判定しない | 先に box で切ってから当てる |
| RG-17-03（P1） | 変形前の寸法が `box-sizing` と `zoom` を反映しない | 寸法は `offsetWidth`/`offsetHeight`、`zoom` は写像へ入れる |
| RG-17-04（P1） | 影・背景の抜き・覆いを「指定の有無」「先頭の層」で判定 | 影は色、背景の抜きは実際の塗り、覆いは**全層**を見る |
| RG-17-05 | 固定名の OFF 属性を、値に関わらず書き換え・削除する | 目印の**名前ごと**読み込みごとに変える |
| RG-17-06 | 合言葉の class まで消された複製が停止点として残る | 名乗りの無い同形の空要素から、操作性だけを外す |
| RG-17-07 | カーソルの合図の費用が候補数に比例したまま | 使ってよい時間に上限（2秒あたり200ms）を置く |
| RG-17-08 | 変形に捕まった fixed を画面固定として扱う | 包含ブロックを作る祖先を見て、画面固定と区別する |

**8件すべて、直す前に v1.8.15 の実物で再現しました。** 通算17巡で 139 件の指摘があり、事実と異なるものは 0 件です。

> **前回の私の報告を訂正します。** 第16回の対応で「RG-16-09 の『5,000候補・10移動で 70,000回』は実測 10,002回だった」と書きましたが、それは**カーソルが同じ要素の中を動く見本**での値でした。要素の境目をまたぐ動かし方にすると v1.8.15 は **75,085回**で、指摘の値とほぼ一致します。**指摘の規模が正しく、私の見本が合図を出していませんでした。**
...
**新しい試験に判別力があることも確かめています。** v1.8.15 の実装（`src/content.js` をタグの内容へ戻したもの）へぶつけると、**26件中17件が失敗**します。通る9件は**すべて落としすぎを見張る対照**です。

> **自分で足した対照が、自分の直しの穴を捕まえました。** RG-17-05 で「ページが置いた目印を**消さない**」ようにしたのですが、OFF にする側では**上書き**していました。ON/OFF を往復する対照で落ちて気づき、目印の名前ごと読み込みごとに変える形へ直しています。

> **検査の件数は `state` によって変わります。** タグを打つ前は **293 件**、`tagged` へ進めるとタグとの突き合わせ4件が加わり **296 件**になります。件数だけを見て「検査を足した／減らした」と読まないでください。

### 4-1b. 性能（2026-08-12）

**① 初回走査**（実行順は交互に反転・切り取りだらけの最悪の形のページ・400行）:

| | v1.8.15 | v1.8.16 |
|---|---|---|
| 4組の実測 | 21.2 / 21.7 / 17.2 / 21.9 ms | 23.3 / 22.0 / 21.8 / 15.9 ms |
| 中央値 | 21.7 ms | 22.0 ms |

**差は付きませんでした**（中央値 +0.3 ms。同じ版の中のばらつきのほうが大きい）。

**② カーソルを動かしたときの費用**（RG-17-07）。**要素の境目を毎回またぐ**動かし方で、`Element.checkVisibility` の呼び出し回数を数えました（陽性対照＝1回で確認済み）。
...
## 6. 権限・通信・保存データ（v1.8.15 → v1.8.16 の差分）
...
| 15 | **合言葉の class まで消された複製は、形で見分ける**（v1.8.16） | 名乗りが無く・空で・押せる同形の `<sup>` から操作性だけを外す。ページがこの形の要素を自分で作れば巻き込むが、外すのは操作性だけで、この形の見えない停止点はページが自分だけでも作れる |
...
| **今回の変更の詳細と証拠** | [`docs/audit/v1.8.16-changes.md`](docs/audit/v1.8.16-changes.md) |
