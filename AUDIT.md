...
| 項目 | 値 |
|---|---|
| **監査対象** | **`v1.8.12` = コミット `5afa0e284a199377902067df9c3cb582ec9b2051`**（タグ済み・提出前） |
| 直前の版 | `v1.8.11` = コミット `92bd593e35839b448f97b051ab427e7a1384c398`（**第13回監査の指摘により提出しない**） |
| 基点コミット | `44004a108e76366e8e9f51fd33e4c6b702672c1b`（ここからの差分が今回の変更。PR #22 の8コミット） |
| 現在の既定ブランチ | `main` |
| Manifest | v1.8.12 / Manifest V3 |
...
| ストア公開中の版 | **v1.7.1**（v1.8.0〜v1.8.12 はいずれも未提出） |

差分は次のコマンドで確認できます。

```sh
git checkout v1.8.12                     # 監査対象を手元に出す
git diff --name-only v1.8.11 v1.8.12     # 直前の版との差分
...
base_commit:        44004a108e76366e8e9f51fd33e4c6b702672c1b
commit:             5afa0e284a199377902067df9c3cb582ec9b2051
tag:                v1.8.12
candidate_zip:      repogloss-1.8.12.zip
candidate_bytes:    118968
candidate_sha256:   a107c2119ccc1c3b31b32699300d5ea73a1e85549339418ad31d543fb75018bd
content_sha256:     99447ee2a2c15c9fc8d96786e2b48596b15d9b0cd063c4ffd72e15d107f9e4bb
combined_sha256:    5e86f647b674ac4336060b6a9e9219e6429882427bf46a44b99b1f4a65a5a268
workflow_run:       31468465677
superseded:
  - version: 1.8.11
    tag: v1.8.11
...
第13回監査の指摘 **P1 1件・P2 6件**への対応。詳細と証拠は [`docs/audit/v1.8.12-changes.md`](docs/audit/v1.8.12-changes.md)。

| ID | 内容 | 主な変更箇所 |
|---|---|---|
| RG-13-01 | 語の実際の描画領域と、祖先の切り取りを照合していない（**P1**） | 切り取りを viewport 座標の矩形として積み上げ、語の矩形との交わりで決める（`paintChain` / `isPaintedText`） |
| RG-13-02 | `:hover` / `:focus-within` だけで短時間ひらく場所の語が説明されない | カーソルとフォーカスも控えの見直しの合図にする（1フレームに1回・控えが空なら何もしない） |
| RG-13-03 | 複製の判定が辞書の説明文に依存し、ページ側の要素まで消していた | 判定を今回の合言葉だけにし、中身のある節点は消さず名札を外す。見た目の絞り込みを吹き出し・切替ボタンへも広げる |
| RG-13-04 | 生成した印の生命周期と意味の照合が足りない | 自分が外す削除だけを1回受け取る（`expectedRemovals`）。「作ったことがある」記録を廃止。`isCoherent` が説明文・用語・役割まで確かめる |
| RG-13-05 | 属性の予定が消費されず残る | 予定を待ち行列にして、変更前の値で1回だけ受け取る。あわせて **`<html>` の属性も見張る**（body だけでは1件も届いていなかった） |
| RG-13-06 | 控えの上限の切り捨てと、周期的な長い処理 | 見直しを 8ms の予算で区切って持ち越す。上限の旗を実際の入れ直しへつなぐ |
| RG-13-07 | 配布 `DESIGN.md` の既知の限界が現行と食い違う | §5 を現況へ書き直し、旧仕様の断定文を検査で禁止する |

**7件すべて、実物で再現してから直しました。** 通算13巡で 103 件の指摘があり、事実と異なるものは 0 件です。

**RG-13-05 の症状は実在しましたが、原因は指摘と違いました。** 反例（`<html>` の class を戻す）で起きていたのは「予定が消費されない」ことではなく、**`<html>` の属性変更が1件も観測されていない**ことでした（`observer` は `document.body` と `document.head` しか見ていません。実測: 表示が消えてもまとめ直しは1回も走らず、暇なときの確認まで約2秒かかりました）。両方を直しています。

**RG-13-06 の上限の扱いは、求められた形と違います。** 「20,001件目を失わない」ではなく、**空きができたときに入れ直す**形にしました。控えが一度も減らない頁では、超えた分はなお説明されません（→ §7 の未検証）。
...
| `npm test` | 0 | 単体 **63件**全成功 / 構成検査 **279項目**・不一致0（辞書61語・version 1.8.12） |
...
> **検査の件数は `state` によって変わります。** タグを打つ前は **276 件**、`tagged` へ進めるとタグとの突き合わせ4件が加わり **279 件**になります（代わりに「この状態では求めない」の1件が走らなくなります）。件数だけを見て「検査を足した／減らした」と読まないでください。

### 4-1b. 性能（2026-08-11）

初回走査を v1.8.11 と比べました（**実行順は交互に反転**。片方を先に測ると、その差が版差に見えるため）。

| 何を | v1.8.11 | v1.8.12 | 判定 |
|---|---|---|---|
| 初回走査・切り取りのあるページ（400行・全行 `overflow:hidden`・1/3 に `clip-path`・6組） | 中央値 **15.2 ms** | 中央値 **16.7 ms** | **6組とも新版が遅い**（+1.5 ms） |
| 初回走査・切り取りの無いページ（400行・4組） | 中央値 **19.0 ms** | 中央値 **18.9 ms** | 差を判定できない（符号が混在） |
| 控え 20,000 件の1回あたり | **32.7〜60.6 ms** | **8.3 ms** | 新版が速い（8ms の予算で区切ったため） |

初回走査が遅くなるのは、**祖先に切り取りがある場所でだけ**語の矩形を測るためです。切り取りの無いページでは箱の大きさだけで済ませるので、費用は変わりません（上の2行目）。切り取りだらけのページは意図的に作った最悪の形で、実サイトでの計測ではありません。

計測は隔離された世界の中から行いました（`MutationObserver.prototype.observe` を包み、**見張り始めてから `<head>` を見張るまで**の間隔を測る＝初回走査の区間。この順序は `verify.mjs` が固定しています）。陽性対照として、包みが効いていることと両版が同数の印を付けることを同じ実行で確認しています。

### 4-2. CI（`main` の run [`31468465677`](https://github.com/Driedsandwich/repogloss/actions/runs/31468465677)・2026-08-11）

**8ジョブすべて success。** マージコミット `5afa0e28…`（＝タグ `v1.8.12` が指すもの）に対する実行です。

| ジョブ | 結果 |
|---|---|
| `verify` | 構成検査 **276項目**・不一致0（当時は `state: uncommitted`） |
| `e2e`（ubuntu・実 Chrome） | **193件**全成功 |
| `e2e-windows`（windows・実 Chrome） | **193件**全成功 |
| `package-hash` ubuntu / macos / windows | 3 OS とも配布13ファイルの合算が `5e86f647…` |
| `hash-compare` | `集まった数: 3` → `OK: 3 OS すべてで同じ` |
| `release-zip` | 提出候補 `repogloss-1.8.12.zip`（`a107c211…`）を生成 |

**PR #22 の段階でも同じ8ジョブが success**でした（run `31468074041`。`release-zip` は `main` への push 限定なので skip）。

---

## 6. 権限・通信・保存データ（v1.8.11 → v1.8.12 の差分）
...
| 構成と文書の整合検査（279項目） | [`scripts/verify.mjs`](scripts/verify.mjs) |
...
| **今回の変更の詳細と証拠** | [`docs/audit/v1.8.12-changes.md`](docs/audit/v1.8.12-changes.md) |
