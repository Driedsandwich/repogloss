...
| 20 | **Windows の E2E が、まれに Chrome の起動待ちで落ちる**（v1.8.17 で初めて観測） | 2026-08-14、`main` の run [`31811058081`](https://github.com/Driedsandwich/repogloss/actions/runs/31811058081) で `e2e-windows` が **1件失敗**した（`CDP タイムアウト: Extensions.loadUnpacked`・28.4秒。CDP の上限は20秒）。**製品側の欠陥ではありません**——落ちたのは Chrome を起動して拡張を読み込む工程で、**同じ配布13ファイル**（合算 `5f5cb620…`）に対する前後の run [`31809124124`](https://github.com/Driedsandwich/repogloss/actions/runs/31809124124) と [`31811253302`](https://github.com/Driedsandwich/repogloss/actions/runs/31811253302) は**どちらも 292件全成功**でした。直近6回のうち1回。親が落ちると配下3件が走らないため、その回だけ件数が 292 → 289 に見えます。**上限を伸ばすか、起動だけ再試行するかは次の版で扱います**（この巡では試験の道具を触っていません） |
