...
| 項目 | 値 |
|---|---|
| **監査対象** | **`v1.8.19` = コミット `d76a6cdb68d5ab5e0fac5895934010f66e585295`**（タグ済み・提出前） |
...
tag:                v1.8.19
candidate_zip:      repogloss-1.8.19.zip
candidate_bytes:    151862
candidate_sha256:   19b5ed38fef1b4f2913740b96f10375201d3c7c5344188f99f7bc801d05db8d6
content_sha256:     ffca78bc396784bb6b0ac676594485cb6d11e02881ed2e0ff9f9e48c6bc83250
combined_sha256:    401133a5255e7bffdd489e30540dfe7af966e5afb9ec54d898309ab3552f7f48
workflow_run:       31866299353
superseded:
  - version: 1.8.18
    tag: v1.8.18
...
取り下げた版のタグと ZIP は、すべて保存したまま動かしていません（`v1.8.19` を打った前後で、既存23タグ＝46 ref が1文字も動いていないことを `refs/tags` 一覧の突き合わせで確認済み。1件の改変なら検出できることも、対照として同じ実行で示しています）。**直前の `v1.8.18`（`b66312dc…`）は、第20回監査の指摘により提出しません。**

**提出候補は、タグの中身と1バイトも違いません。** CI の run `31866299353` の `release-zip` が出した成果物を落として展開し、13ファイルを `git show v1.8.19:<path>` と突き合わせて **13/13 一致**。`--release` 検査にも合格しています（実測: `EXIT=0`）。

**ZIP は誰でも再現できます。** 日時を 1980-01-01 に固定し、並び順を配布一覧の順に固定してあるため、同じ内容からは1バイト違わない同じものができます。配布13ファイルの合算ハッシュは **ubuntu / macos / windows / 手元 macOS の4者で完全に一致**しています（`401133a5…`）。

> **⚠️ ZIP のハッシュだけでは、CI の成果物と手元ビルドを区別できません。** ビルドが決定的なので、同じ内容からは同じ ZIP ができます（今巡も実測: 手元ビルドと CI の成果物は**バイト単位で同一**の `19b5ed38…`）。区別が付くのは **sidecar だけ**です——検査したのは `builtInCI: true` / `workflowRunId: 31866299353` を持つほうです。手元ビルドの sidecar を正本に据えると、同じ ZIP でも `--release` 検査は落ちます（実測: `EXIT=1`・「提出候補は CI で作ったものに限る（builtInCI が true でない）」）。
...
| `npm test` | 0 | 単体 **63件**全成功 / 構成検査 **337項目**・不一致0（辞書61語・version 1.8.19） |
| `npm run test:e2e` | 0 | **353件**全成功（v1.8.18 は 325件。**+28件**） |
| `npm run package:stage` / `:verify` | 0 | 13ファイル一致 |
| `npm run package:verify-zip -- --release`（CI の成果物） | 0 | 13ファイル / 151,862 バイト（`19b5ed38…`） |
...
### 4-2. CI（`main` の run [`31866299353`](https://github.com/Driedsandwich/repogloss/actions/runs/31866299353)・2026-08-15）

**8ジョブすべて success。** マージコミット `d76a6cdb…`（＝タグ `v1.8.19` が指すもの）に対する実行です。

| ジョブ | 結果 |
|---|---|
| `verify` | 構成検査 **334項目**・不一致0（当時は `state: uncommitted`） |
| `e2e`（ubuntu） / `e2e-windows` | どちらも **353件**全成功 |
| `package-hash` 3 OS / `hash-compare` | 合算が `401133a5…` で3 OS 一致（`OK: 3 OS すべてで同じ`） |
| `release-zip` | 提出候補 ZIP（`19b5ed38…`）を生成 |

PR #29 の CI（run `31865962379`）も同じジョブが success でした（`release-zip` は skip）。
...
| 構成と文書の整合検査（337項目） | [`scripts/verify.mjs`](scripts/verify.mjs) |
