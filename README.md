...
スクリーンショットは未掲載です（実機での確認が済んでいないため）。文章で書くと、GitHub のナビゲーションはこう見えます。

```
Code    Pull requests ⓘ    Issues ⓘ    Actions ⓘ    Projects ⓘ    Wiki ⓘ
```

ⓘ にカーソルを乗せると、その語の説明が吹き出しで出ます。
...
Chrome ウェブストアには**未提出**です。現時点の導入方法は手動読み込みのみです。
...
個人が開発している非公式ツールです。**まだ実際の使用による検証を経ていません。** 導入を判断する材料として、現時点の状況を書きます。
