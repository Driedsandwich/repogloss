...
**[Chrome ウェブストアで公開しています](https://chromewebstore.google.com/detail/ihkkhkleamggokepaelapoiabgmpljnn)**。ストアに出ているのは v1.7.1（2026-08-03 公開）で、この版 v1.8.5 はまだ提出していません。
...
- 大きなページでの速度 — 用語が多い `k88hudson/git-flight-rules` で、初期走査の中央値は **62 ミリ秒**、最小 53 ミリ秒（v1.8.5・macOS・Google Chrome 151 の headless・幅 1600px・v1.8.4 と交互に5往復・ページの中で計測）。**このとき機械が混んでおり、比較対象の v1.8.4 も 61 ミリ秒でした**（空いていた時間帯の同じページは v1.8.4 で 36 ミリ秒）。**目安にしている 50 ミリ秒を、この測定では示せていません**（2026-08-06 実測・条件と経緯は [DESIGN.md](./DESIGN.md)）
