...
- 大きなページでの速度 — 用語が多い `k88hudson/git-flight-rules` で、最初の走査に **26 ミリ秒**（v1.8.3・macOS・Google Chrome 151 の headless・幅 1600px・5回の中央値・ページの中で計測）。同じ条件で `octocat/Hello-World` は 11 ミリ秒。**50 ミリ秒を超える処理はありません**（2026-08-06 実測・条件と旧版との比較は [DESIGN.md](./DESIGN.md)）
